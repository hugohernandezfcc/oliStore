<template>
    <div class="bg-white p-3 m-2 border-red-600 border">
        <el-row>
            ...
            <el-col :span="14"><span class="text-lg font-bold text-red-600 ">{{task.assigned_to.name}}</span></el-col>
            <el-col :span="8" class="align-right">
                <el-icon :size="15" :color="'#ec0505'" class="m-1" v-if="showAvatar">
        ...
        <el-row>
            <el-col :span="24">
                ...
                <span> Tarea de <b class="text-red-600 ">{{task.assigned_to.name}}</b></span> || <span :class="`bg-${(task.status == 'open' || task.status == 'Tarea abierta') ? 'green' : 'red'}-600 p-1 text-white`"> {{task.status}}</span>
            </el-col>
        </el-row>
        <br/>
        <div class="bg-gray-100 p-3 -m-3">
            <span class="pb-1 text-xs"> Creado por <b class="text-red-600 ">{{task.created_by.name}}</b></span>
            <Posts  :target_id="task.id" :key="index" v-if="withComments"/>
        </div>
    </div>

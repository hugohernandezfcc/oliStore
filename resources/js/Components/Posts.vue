    ...
    <div v-for="item in items" :key="item.id">
        <el-row>
            <el-col :span="20">
                ...
                <div class="m-1" >
                  <el-icon :size="20" :color="'#3a953ad6'" >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" data-v-ea893728=""><path fill="currentColor" d="M736 504a56 56 0 1 1 0-112 56 56 0 0 1 0 112m-224 0a56 56 0 1 1 0-112 56 56 0 0 1 0 112m-224 0a56 56 0 1 1 0-112 56 56 0 0 1 0 112M128 128v640h192v160l224-160h352V128z"></path></svg>
                  </el-icon>
                  {{ item.comment }}
                </div>
            </el-col>
        ...
        <el-divider content-position="left"><b>Fecha/hora:</b> {{ item.datetime }}</el-divider>
  ...
  export default {
    ...
    methods: {
      createPost() {
        ...
        this.textarea.length < 1 ? alert('No puedes enviar un comentario vacío') : axios.post(route('social.create.post'), {
          ...
          this.items.push({
            ...
            avatar: response.data.user_data.profile_photo_url,
        ...
        axios.post(route('social.posts', {
          ...
          for (let i = 0; i < response.data.length; i++) {
            this.items.push({
              ...
              name: response.data[i].posted_by.name,
              comment: response.data[i].content,
              avatar: response.data[i].posted_by.profile_photo_url,
              datetime: response.data[i].created_at
            })
          }
          this.items = [...this.items].reverse();
      ...
      IlikeIt() {
        ...
        axios.post(route('social.like.post'), {

<template>
    ...
    <div class="grid grid-cols-8 gap-4 shadow-lg bg-white rounded-lg mx-2 my-1  px-2 py-1">
        ...
        <!--   @click="sendByWhatsapp(product.name, product.description, product.id)" v-if="account.manager"-->
        <div class="p-2 col-span-6">
            <span class="font-bold text-green-600 pr-24">$ {{ product.price }} MXN</span>
                <el-button  size="small"  @click="() => {
                    console.log(account.manager);
                    if(account.manager){
                        openClients(product.name, product.description, product.id);
                    }else{
                        askForProduct(product.name, product.description, product.id);
                    }
                }">
    ...
    <el-dialog v-model="dialogTableVisible" width="100%"  :show-close="false" >
        ...
        <el-table :data="gridData">
            ...
            <el-table-column label="Detalle de la tienda" width="190" >
                <template #default="scope">
                    <span class="font-bold">- {{ scope.row.account.name }} </span> <br/> - {{ scope.row.firstname }} {{ scope.row.lastname }}
                </template>
            </el-table-column>
    ...
    methods: {
        openClients(title, Description, idProduct){
            ...
            axios.get('/app/ecommerce/accounts/with/contacts').then(response => {
                ...
                this.gridData = response.data.accounts;
...
${Description}`;
        ...
        sendByWhatsapp(title, Description, idProduct, phoneLocal){
